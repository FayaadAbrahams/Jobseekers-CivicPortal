...
import cityLogo from "@/assets/imgs/coct-logo.svg";
interface Message {
  id: string;
  from: "bot" | "user";
  text: string;
...
const BOT_RESPONSES: Record<string, { text: string; followUp?: QuickReply[] }> =
  {
    ...
    human: {
      ...
      followUp: [{ label: "Start over", value: "restart" }],
    },
    restart: {
...
export default function WhatsAppChatbot({
  ...
  const allReplies = showMore
    ? [
        ...quickReplies,
        ...QUICK_REPLIES_MORE.filter(
          (r) => !quickReplies.some((q) => q.value === r.value),
        ),
      ]
    : quickReplies;
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-50 w-85 max-w-[calc(100vw-20px)] rounded-2xl overflow-hidden shadow-2xl flex flex-col"
          ...
          >
            ...
            >
              <div
                ...
                style={{
                  background: isHigh ? "#FFD700" : "#25D366",
                  color: isHigh ? "#000" : "#fff",
                }}
              >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-green-200 text-[16px] font-medium">Online</p>
                <a className="" href="tel:+27 123 456 7890">
                  +27 123 456 7890
                </a>
              </div>
              <button
            ...
            {!typing && quickReplies.length > 0 && (
              <div
                className="shrink-0 px-3 pt-2 pb-1 flex flex-wrap gap-1.5 border-t"
                style={{
              ...
              >
                {allReplies.map((qr) => (
                  <button
                    ...
                    className="text-[16px] font-semibold px-2.5 py-1.5 rounded-full border cursor-pointer transition-all hover:opacity-80"
                    style={{
            ...
            <div
              ...
              style={{
                background: isHigh ? "#000" : "#f0f0f0",
                borderTop: `1px solid ${isHigh ? "#333" : "#e5e7eb"}`,
              }}
            >
              <Send size={14} style={{ color: isHigh ? "#666" : "#bbb" }} />
              <span
                className="text-[16px]"
                style={{ color: isHigh ? "#666" : "#aaa" }}
              >
