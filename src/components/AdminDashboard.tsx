...
import { 
  Building, Briefcase, Plus, Edit, Trash, Users, Search, DollarSign, MapPin, 
  Clock, Filter, LogOut, CheckCircle, AlertCircle, XCircle, Calendar, 
  SlidersHorizontal, Check, RefreshCw, FileText, UserCheck, Inbox, ArrowLeft, ArrowUpRight
} from 'lucide-react';
...
export default function AdminDashboard({
  ...
  const [jobCategory, setJobCategory] = useState<Job['category']>('Administration');
  ...
  const [appStatusFilter, setAppStatusFilter] = useState<'All' | 'applied' | 'reviewing' | 'interview_scheduled' | 'action_required' | 'placed' | 'rejected'>('All');
  ...
  const handleJobSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!jobTitle || !jobDept || !jobLocation || !jobDeadline || !jobSalary || !jobDesc) {
  ...
  const handleDeleteJob = (id: string) => {
    if (confirm('Are you absolutely certain you want to remove this job placement vacancy? All existing applications associated with it will remain referenced.')) {
  ...
  const handleUpdateAppStatus = (appId: string, status: Application['status'], extra?: Partial<Application>) => {
    ...
    if (app && job) {
      ...
      try {
        const notifs = JSON.parse(localStorage.getItem('cp_notifications') || '[]');
        localStorage.setItem('cp_notifications', JSON.stringify([newNotification, ...notifs]));
      } catch {}
    }
  };
  ...
  const handlePlacementSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedApp || !placeStartDate || !placeReporting || !placeLocation || !placeMessage) return;
  ...
  const filteredCitizens = allCitizens.filter(cit => {
    return cit.fullName.toLowerCase().includes(citizenQuery.toLowerCase()) || 
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans text-slate-800">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <h1 className="text-2xl font-black tracking-tight flex items-center gap-2">
                  CivicPortal <span className="text-xs font-semibold text-indigo-400 bg-indigo-950 px-2.5 py-0.5 rounded-full uppercase tracking-wider">Admin</span>
                </h1>
              </div>
              <p className="text-xs text-slate-400 mt-1 font-medium">National Placement Agency Clearinghouse &bull; Authorizing Officer Panel</p>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* System Stats Row - Vibrant Design with rounded-[32px] bento shapes and bright gradients */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-indigo-600 text-white p-6 rounded-[32px] shadow-xl shadow-indigo-100 flex items-center justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-indigo-100">Active Jobs</p>
          ...
          <div className="bg-teal-500 text-white p-6 rounded-[32px] shadow-xl shadow-teal-55 flex items-center justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-teal-55">Registered Users</p>
          ...
          <div className="bg-amber-400 text-slate-950 p-6 rounded-[32px] shadow-xl shadow-amber-50 flex items-center justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-slate-900/60">Submissions</p>
              <h3 className="text-4xl font-black mt-2">{allApplications.length}</h3>
            </div>
          ...
          <div className="bg-rose-500 text-white p-6 rounded-[32px] shadow-xl shadow-rose-100 flex items-center justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-rose-100">Placements</p>
        ...
        {activeTab === 'jobs' && (
          <div className="space-y-6" id="admin-jobs-tab">
            ...
            <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-white p-5 rounded-[24px] border border-slate-150 shadow-xs">
                ...
                >
                  <div className="flex justify-between items-center pb-4 border-b border-slate-100 mb-6">
                    <h4 className="text-base font-bold text-slate-900 flex items-center">
                      ...
                      {editingJobId ? 'Modify Opportunity Parameters' : 'Register New Placement Vacancy'}
                    </h4>
                    <button 
                  ...
                  <form onSubmit={handleJobSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Official Title *</label>
                      <input 
                        id="form-title"
                        type="text" 
                        required
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Authorizing Department *</label>
                      <input 
                        id="form-dept"
                        type="text" 
                        required
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Regional Location *</label>
                      <input 
                        id="form-location"
                        type="text" 
                        required
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Estimated Salary Packages *</label>
                      <input 
                        id="form-salary"
                        type="text" 
                        required
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Contract Classification</label>
                      <select 
                        id="form-type"
                        value={jobType}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Sector Classification</label>
                      <select 
                        id="form-category"
                        value={jobCategory}
                      ...
                      >
                        ...
                        <option value="Skilled Labor">Skilled Labor & Trades</option>
                        <option value="Community Service">Community Service</option>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Target Audience Accents</label>
                      <input 
                        id="form-target"
                        type="text"
                        value={jobTarget}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Submission Deadline *</label>
                      <input 
                        id="form-deadline"
                        type="date" 
                        required
                    ...
                    <div className="md:col-span-2">
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Detailed Description *</label>
                      <textarea 
                        id="form-description"
                        required
                        rows={3}
                    ...
                    <div className="md:col-span-2">
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Mandatory Entry Requirements (One per line)</label>
                      <textarea 
                    ...
                    <div className="md:col-span-2 flex justify-end space-x-3 pt-2">
                      ...
                      >
                        {editingJobId ? 'Save Core Updates' : 'Confirm & Publish Vacancy'}
                      </button>
                    </div>
                  </form>
                </motion.div>
            ...
            {/* Vacancy Card List */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="admin-job-cards-container">
              {filteredJobs.length === 0 ? (
                <div className="col-span-full bg-white text-center py-12 px-6 rounded-[32px] border border-slate-155 text-slate-400">
                  <Briefcase size={40} className="mx-auto mb-3 opacity-30" />
                  <p className="font-semibold text-slate-700">No Job Listings Match Criteria</p>
                  <p className="text-xs text-slate-400 mt-1">Try relaxing queries, filters, or publish a new opportunity.</p>
                </div>
              ) : (
                filteredJobs.map(job => (
                  <div 
                    key={job.id} 
                    className="bg-white p-6 rounded-[32px] border-2 border-transparent hover:border-indigo-400 transition-all shadow-xs hover:shadow-md flex flex-col justify-between"
                  >
                    <div>
                      ...
                      <p className="text-xs text-slate-500 font-bold mt-1 flex items-center">
                        <Building size={13} className="mr-1 inline text-indigo-550" />
                        {job.department}
                      </p>
        ...
        {activeTab === 'citizens' && (
          <div className="space-y-6" id="admin-citizens-tab">
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4 bg-white p-5 rounded-[24px] border border-slate-150 shadow-xs">
              ...
              <div className="text-xs text-indigo-800 bg-indigo-50 font-bold px-4 py-2.5 rounded-xl border border-indigo-150 uppercase tracking-wider">
                Registered Residents: <strong className="text-indigo-950">{filteredCitizens.length}</strong>
              </div>
            </div>

            {/* List and Grid */}
            <div className="bg-white rounded-[32px] border border-slate-150 shadow-xl shadow-slate-100 overflow-hidden">
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-150" id="citizens-directory-table">
                  <thead className="bg-slate-55">
                    <tr>
                      <th scope="col" className="px-6 py-3.5 text-left text-xs font-bold text-slate-500 uppercase tracking-widest">
                        Full Citizen Name
                      </th>
                      <th scope="col" className="px-6 py-3.5 text-left text-xs font-bold text-slate-500 uppercase tracking-widest">
                        ID Number
                      ...
                      <th scope="col" className="px-6 py-3.5 text-right text-xs font-bold text-slate-500 uppercase tracking-widest">
                        Operations
                      </th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-slate-100">
                    {filteredCitizens.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="text-center py-12 px-4 text-slate-450 text-xs">
                          <Users size={32} className="mx-auto text-slate-300 mb-2" />
                          No registered individuals found. Complete a login attempt in the user tab to register.
                        </td>
                      </tr>
                    ) : (
                      filteredCitizens.map(cit => {
                        const citApps = allApplications.filter(a => a.citizenIdNumber === cit.idNumber);
                        return (
                          <tr key={cit.idNumber} className="hover:bg-slate-50/50 transition-all">
                            <td className="px-6 py-4.5 whitespace-nowrap">
                              <div className="flex items-center space-x-3">
                                ...
                                <div>
                                  <div className="text-sm font-bold text-slate-900">{cit.fullName}</div>
                                  <div className="text-xs text-slate-450">{cit.gender || 'Not specified'}</div>
                                </div>
                              </div>
                            </td>
                            ...
                            <td className="px-6 py-4.5 whitespace-nowrap text-xs">
                              <div className="text-slate-800 font-medium">{cit.phone || '+27 General Line'}</div>
                              <div className="text-slate-400">{cit.email || 'not.linked@digital.gov'}</div>
                            </td>
                  ...
                  >
                    <div className="bg-slate-900 text-white p-6 flex justify-between items-center">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h3 className="text-base font-bold">{selectedCitizen.fullName}</h3>
                          <p className="text-xs text-slate-400">National ID File: {selectedCitizen.idNumber}</p>
                        </div>
                      </div>
                      <button 
                    ...
                    <div className="p-6 space-y-6">
                      <div className="grid grid-cols-2 gap-4 text-xs">
                        <div>
                          <p className="text-slate-400 uppercase font-semibold tracking-wider">Mobile Contact</p>
                          <p className="text-slate-850 font-bold mt-0.5">{selectedCitizen.phone || 'None Spec.'}</p>
                        </div>
                        <div>
                          <p className="text-slate-400 uppercase font-semibold tracking-wider">Digital Mail</p>
                          <p className="text-slate-850 font-bold mt-0.5">{selectedCitizen.email || 'None Spec.'}</p>
                        </div>
                        <div>
                          <p className="text-slate-400 uppercase font-semibold tracking-wider">Gender Profile</p>
                          <p className="text-slate-850 font-bold mt-0.5">{selectedCitizen.gender || 'Not declared'}</p>
                        </div>
                        <div>
                          <p className="text-slate-400 uppercase font-semibold tracking-wider">Date of First Clearance</p>
                          <p className="text-slate-850 font-mono font-semibold mt-0.5">{selectedCitizen.registeredDate}</p>
                        </div>
                      </div>

                      <div className="border-t border-slate-100 pt-5 text-xs">
                        <p className="text-slate-400 uppercase font-semibold tracking-wider mb-2">Capabilities & Skill Accents</p>
                        <div className="flex flex-wrap gap-1.5">
                          {selectedCitizen.skills && selectedCitizen.skills.length > 0 ? (
                            selectedCitizen.skills.map(s => (
                              <span key={s} className="bg-slate-100 text-slate-850 font-semibold px-2.5 py-1 rounded-lg border border-slate-200">
                                {s}
                              </span>
                            ))
                          ) : (
                            <span className="text-slate-400 italic">No custom skills specified. Default citizen clearances set.</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="border-t border-slate-100 pt-5">
                        ...
                        <div className="space-y-3">
                          {allApplications.filter(a => a.citizenIdNumber === selectedCitizen.idNumber).length === 0 ? (
                            <p className="text-xs text-slate-400 italic py-1">This citizen hasn't applied for any vacancies yet.</p>
                          ) : (
                            allApplications
                              .filter(a => a.citizenIdNumber === selectedCitizen.idNumber)
                              .map(app => {
                                const job = allJobs.find(j => j.id === app.jobId);
                                return (
                                  <div key={app.id} className="bg-slate-50 p-3 rounded-xl border border-slate-200 flex justify-between items-center text-xs">
                                    <div>
                                      <p className="font-bold text-slate-850">{job?.title || 'Unknown Vacancy'}</p>
                                      <p className="text-slate-450 text-[11px] font-semibold mt-0.5">{job?.department}</p>
                                    </div>
                                    <span className={`px-2 py-0.5 rounded text-[11px] font-bold uppercase ${
                                      ...
                                      {app.status === 'interview_scheduled' ? 'Interview Set' : app.status}
                                    </span>
                                  </div>
                                );
                              })
        ...
        {activeTab === 'applications' && (
          <div className="space-y-6" id="admin-applications-tab">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-[24px] border border-slate-150 shadow-xs">
              <div>
                <h3 className="text-base font-extrabold text-slate-950">Enrollment Cleared Submissions</h3>
                <p className="text-xs text-slate-400 mt-0.5">Control pipeline states from initial screen to direct employment letters.</p>
              </div>

              <div className="flex flex-wrap gap-2">
                {(['All', 'applied', 'reviewing', 'interview_scheduled', 'action_required', 'placed', 'rejected'] as const).map(tab => (
                  <button
                    key={tab}
                    id={`app-filter-${tab}`}
                  ...
                  >
                    {tab === 'interview_scheduled' ? 'Interview' : tab === 'action_required' ? 'Docs Needed' : tab}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-1 bg-white rounded-[24px] border border-slate-150 shadow-xs p-4 space-y-3 max-h-[600px] overflow-y-auto">
                ...
                {filteredApps.length === 0 ? (
                  <div className="text-center py-10 px-4 text-slate-400 text-xs">
                    <Inbox className="mx-auto mb-2 opacity-30 text-slate-400" size={28} />
                      ...
                      >
                        <div className="flex justify-between items-start gap-2">
                          ...
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase shrink-0 ${
                            ...
                            {app.status === 'interview_scheduled' ? 'Scheduled' : app.status === 'action_required' ? 'Alert Docs' : app.status}
                          </span>
                        </div>
                        ...
                        <p className="text-[11px] text-slate-400 mt-2 font-semibold">
                          ID: <span className="font-mono text-slate-600">{app.citizenIdNumber}</span>
                        </p>
                      </button>
                    );
                  })
              ...
              <div className="lg:col-span-2 space-y-4">
                {selectedApp ? (
                  <div className="bg-white rounded-[24px] border border-slate-150 shadow-xs p-6" id="app-inspector-card">
                    ...
                    <div className="pb-5 border-b border-slate-100 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                      <div>
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">Applied: {selectedApp.appliedDate}</p>
                      </div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          id="btn-action-docs"
                          onClick={() => handleUpdateAppStatus(selectedApp.id, 'action_required')}
                          className={`px-3 py-1.5 border rounded-lg text-xs font-semibold tracking-wide transition-all ${
                    ...
                    <div className="mt-5 p-4 bg-slate-50 rounded-xl border border-slate-200/80">
                      <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5">Targets & Placement Vacancy</h4>
                      <div className="flex justify-between items-center text-xs">
                        <div>
                          <strong className="text-slate-800 text-sm">
                            {allJobs.find(j => j.id === selectedApp.jobId)?.title || 'Unknown Job Title'}
                          </strong>
                          <p className="text-slate-450 font-medium mt-0.5">
                            {allJobs.find(j => j.id === selectedApp.jobId)?.department || 'Unknown Dept'}
                          </p>
                        </div>
                        <span className="bg-amber-50 text-amber-800 px-2 py-0.5 rounded-full font-bold text-[11px]">
                          🚨 Targets: {allJobs.find(j => j.id === selectedApp.jobId)?.targetAudience}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="mt-6 space-y-3">
                      <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Document Upload Clearance</h4>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                        <div className={`p-3 rounded-xl border flex items-center justify-between font-semibold ${
                          selectedApp.documents.idCopy ? 'bg-emerald-50 text-emerald-800 border-emerald-100' : 'bg-red-50 text-red-800 border-red-100'
                        }`}>
                          <span>National ID Copy</span>
                          {selectedApp.documents.idCopy ? <Check size={14} /> : <AlertCircle size={14} />}
                        </div>
                        <div className={`p-3 rounded-xl border flex items-center justify-between font-semibold ${
                          selectedApp.documents.cv ? 'bg-emerald-50 text-emerald-800 border-emerald-100' : 'bg-red-50 text-red-800 border-red-100'
                        }`}>
                          <span>Curriculum Vitae</span>
                          {selectedApp.documents.cv ? <Check size={14} /> : <AlertCircle size={14} />}
                        </div>
                        <div className={`p-3 rounded-xl border flex items-center justify-between font-semibold ${
                          selectedApp.documents.qualifications ? 'bg-emerald-50 text-emerald-800 border-emerald-100' : 'bg-red-50 text-red-800 border-red-100'
                        }`}>
                          <span>Qualifications Check</span>
                          {selectedApp.documents.qualifications ? <Check size={14} /> : <AlertCircle size={14} />}
                        </div>
                        <div className={`p-3 rounded-xl border flex items-center justify-between font-semibold ${
                          selectedApp.documents.proofOfAddress ? 'bg-emerald-50 text-emerald-800 border-emerald-100' : 'bg-red-50 text-red-800 border-red-100'
                        }`}>
                          <span>Address Clearance</span>
                          {selectedApp.documents.proofOfAddress ? <Check size={14} /> : <AlertCircle size={14} />}
                        </div>
                      </div>
                    </div>

                    {/* Scheduling Details / Interview information if exists */}
                    {allInterviews.some(i => i.applicationId === selectedApp.id) && (
                      <div className="mt-6 p-4 bg-blue-50 border border-blue-150 rounded-xl text-xs">
                        ...
                        {allInterviews.filter(i => i.applicationId === selectedApp.id).map(int => (
                          <div key={int.id} className="grid grid-cols-2 gap-4 mt-2 font-medium">
                            <div>
                              <p className="text-slate-450">Date Time</p>
                              <p className="text-slate-800 font-bold">{int.dateTime.replace('T', ' ')}</p>
                            </div>
                            <div>
                              <p className="text-slate-450">Physical or Digital Location</p>
                              <p className="text-slate-800 font-bold">{int.location}</p>
                            </div>
                            <div>
                              <p className="text-slate-450">HR Officer in Charge</p>
                              <p className="text-slate-800 font-bold">{int.contactPerson}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {selectedApp.status === 'placed' && selectedApp.placementDetails && (
                      <div className="mt-6 p-4 bg-emerald-50 border border-emerald-150 rounded-xl text-xs">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-2">
                          <div>
                            <p className="text-emerald-700/80 font-semibold uppercase tracking-wider text-[10px]">Registry Start Date</p>
                            <p className="font-bold text-slate-850">{selectedApp.placementDetails.startDate}</p>
                          </div>
                          <div>
                            <p className="text-emerald-700/80 font-semibold uppercase tracking-wider text-[10px]">Reporting Officer</p>
                            <p className="font-bold text-slate-850">{selectedApp.placementDetails.reportingTo}</p>
                          </div>
                          <div className="md:col-span-2">
                            <p className="text-emerald-700/80 font-semibold uppercase tracking-wider text-[10px]">Deployment Physical Location</p>
                            <p className="font-bold text-slate-850">{selectedApp.placementDetails.officeLocation}</p>
                          </div>
                          <div className="md:col-span-2 bg-white/70 p-3 rounded-lg border border-emerald-100">
                            <p className="text-emerald-700/80 font-semibold uppercase tracking-wider text-[10px] mb-1">Official Message</p>
                            <p className="italic text-slate-700">{selectedApp.placementDetails.congratulationsMessage}</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="mt-6">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-1">Internal Bureau Notes</label>
                      <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs italic text-slate-600">
                        {selectedApp.interviewerNotes || 'No custom internal bureau comments added yet.'}
                      </div>
                    </div>
                    ...
                    <div className="mt-8 pt-6 border-t border-slate-150 flex flex-wrap gap-3 justify-between items-center">
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setPlaceStartDate(new Date(Date.now() + 14*24*60*60*1000).toISOString().split('T')[0]);
                            const job = allJobs.find(j => j.id === selectedApp.jobId);
                            setPlaceReporting('Direct Line Coordinator');
                            setPlaceLocation(job?.location || 'Regional Office');
                            setPlaceMessage(`Dear ${selectedApp.citizenName}, Your placement is officially authorized. We are absolutely thrilled to welcome you on board.`);
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (confirm('Are you authorized to reject this citizen candidacy?')) {
                  ...
                  <div className="bg-white rounded-2xl border border-slate-150 shadow-xs p-12 text-center text-slate-400">
                    <Inbox size={42} className="mx-auto mb-3 opacity-30 text-slate-400" />
                    <p className="font-semibold text-slate-700">Submissions Pipeline Inspector</p>
                    <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                      Select an active profile card on the left panel to execute status adjustments, issue documents calls, specify interview slottings, or dispatch digital hiring credentials.
                    </p>
                  </div>
                )}
              </div>
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
                  ...
                  >
                    <div className="bg-slate-900 text-white p-5 flex justify-between items-center">
                      <h4 className="font-bold text-sm uppercase tracking-wider">Schedule Official Interview</h4>
                      <button onClick={() => setShowInterviewModal(false)} className="text-slate-400 hover:text-white">✕</button>
                    </div>

                    <form onSubmit={handleScheduleInterviewSubmit} className="p-6 space-y-4 text-xs">
                      <p className="text-slate-500 leading-normal mb-3">
                        Enter scheduled dates to synchronize records. This schedules an online video session or in-office panel directly on the citizen's notification panel.
                      </p>

                      <div>
                        <label className="block text-slate-500 font-semibold mb-1">Calendar Date & Time *</label>
                        <input
                      ...
                      <div>
                        <label className="block text-slate-500 font-semibold mb-1">Location Details *</label>
                        <input
                      ...
                      <div>
                        <label className="block text-slate-500 font-semibold mb-1">Recruiter / Point of Contact *</label>
                        <input
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
                  ...
                  >
                    <div className="bg-emerald-950 text-white p-5 flex justify-between items-center border-b border-emerald-900">
                      <div>
                        <h4 className="font-bold text-sm uppercase tracking-wider">Confirm Official Placement</h4>
                        <p className="text-[10px] text-emerald-400 mt-0.5">Publish official hirable letters for the citizen</p>
                      </div>
                      <button onClick={() => setShowPlacementModal(false)} className="text-slate-400 hover:text-white">✕</button>
                    </div>

                    <form onSubmit={handlePlacementSubmit} className="p-6 space-y-4 text-xs">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-slate-500 font-semibold mb-1">Official Start Date *</label>
                          <input
                        ...
                        <div>
                          <label className="block text-slate-500 font-semibold mb-1">Reporting Officer / Contact *</label>
                          <input
                        ...
                        <div className="col-span-2">
                          <label className="block text-slate-500 font-semibold mb-1">Physical Station / Deployment Address *</label>
                          <input
                        ...
                        <div className="col-span-2">
                          <label className="block text-slate-500 font-semibold mb-1">Congratulations Message & Requirements instructions *</label>
                          <textarea
      ...
      <footer className="bg-slate-900 border-t border-slate-850 text-slate-400 py-6 text-center text-xs mt-12">
        <p>&copy; 2026 Department of Public Enterprise &amp; Citizen Infrastructure Clearinghouse.</p>
        <p className="mt-1 text-[10px] text-slate-600">Administrative Sandbox &bull; All changes saved client side.</p>
      </footer>
