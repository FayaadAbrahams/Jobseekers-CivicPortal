...
interface CitizenDashboardProps {
  ...
  setAllNotifications: React.Dispatch<
    React.SetStateAction<SystemNotification[]>
  >;
  onLogout: () => void;
...
export default function CitizenDashboard({
  ...
  const [citizenEducation, setCitizenEducation] = useState(
    citizen.education || "High School Level",
  );
  const [citizenSkillsSelected, setCitizenSkillsSelected] = useState<string[]>(
    citizen.skills || [],
  );
...
- 4 years active site workshop experience`,
  ...
  const handleCvScanAndExtract = () => {
    if (!cvText.trim()) {
      alert(
        "Please fill in some CV content or select one of our preloaded municipal coordinates to test scans.",
      );
      return;
    }
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const phoneRegex =
            /\+?[0-9]{2,3}[-\s]?[0-9]{2,3}[-\s]?[0-9]{3,4}[-\s]?[0-9]{3,4}/g;
          ...
          setExtractionAlert(
            "CV matching scans completed successfully! Your portfolio details are updated below.",
          );
        }, 900);
      }, 700);
    }, 600);
  ...
  const [uploadedIdFileName, setUploadedIdFileName] = useState<string | null>(
    "National_ID.pdf",
  );
  const [uploadedCvFileName, setUploadedCvFileName] = useState<string | null>(
    null,
  );
  const [uploadedQuailsFileName, setUploadedQuailsFileName] = useState<
    string | null
  >(null);
  const [uploadedAddrFileName, setUploadedAddrFileName] = useState<
    string | null
  >(null);
  ...
  const [cameraDocType, setCameraDocType] = useState<
    "id" | "cv" | "qualifications" | "address" | "cv-scan" | null
  >(null);
  ...
  const [printingPlacement, setPrintingPlacement] =
    useState<Application | null>(null);

  const myApplications = allApplications.filter(
    (app) => app.citizenIdNumber === citizen.idNumber,
  );
  const myInterviews = allInterviews.filter(
    (int) => int.citizenIdNumber === citizen.idNumber,
  );
  const myNotifications = allNotifications.filter(
    (notif) => notif.citizenIdNumber === citizen.idNumber,
  );
  ...
  const handleCameraCapture = (fileName: string) => {
    if (!cameraDocType) return;
    if (cameraDocType === "cv-scan") {
  ...
  const handleUploadMissingDoc = (
    appId: string,
    docType: keyof Application["documents"],
  ) => {
    ...
    localStorage.setItem(
      "cp_notifications",
      JSON.stringify([newNotification, ...allNotifications]),
    );
  };
  ...
  const handleAddSkill = () => {
    if (
      newSkillText.trim() &&
      !citizenSkillsSelected.includes(newSkillText.trim())
    ) {
  ...
  const handleClearAllNotifications = () => {
    const updated = allNotifications.filter(
      (n) => n.citizenIdNumber !== citizen.idNumber,
    );
    setAllNotifications(updated);
    localStorage.setItem("cp_notifications", JSON.stringify(updated));
  };
                      ...
                      <div>
                        ...
                        <div className="mt-4 pt-3.5 border-t border-slate-100 flex flex-wrap gap-1.5">
                          {job.requirements.slice(0, 2).map((req, i) => (
                            <span
                              key={i}
                              className="bg-indigo-50 text-indigo-700 text-[10px] px-2.5 py-0.5 rounded-md font-bold leading-tight"
                            >
                              ✓ {req}
                            </span>
                          ))}
                  ...
                  >
                    ...
                    <div className="bg-slate-900 text-white p-6 flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-indigo-400 mt-1.5 font-bold uppercase tracking-wider">
                        ...
                        {applyWizardStep === 4 ? (
                          ...
                          <>
                            <span
                              className={
                                applyWizardStep === 1
                                  ? "font-black underline text-indigo-900"
                                  : ""
                              }
                            >
                              1. Portfolio Specs
                    ...
                    <div className="p-6 max-h-125 overflow-y-auto space-y-6">
                      {!showApplyModal ? (
                        ...
                        <div className="space-y-5 text-sm leading-relaxed text-slate-600">
                          <div>
                            <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-1.5">
                              Employment Profile
                            </h4>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs text-slate-700">
                            ...
                            <div>
                              <p className="text-slate-455 font-semibold">
                                Contract Class
                              </p>
                              <p className="font-bold text-slate-900 mt-0.5">
                                {selectedJob.type}
                          ...
                          <div>
                            ...
                            <ul className="space-y-1.5">
                              {selectedJob.requirements.map((req, i) => (
                                <li
                                  key={i}
                                  className="flex items-start text-xs text-slate-700"
                                >
                          ...
                          <div className="flex items-center space-x-2 bg-amber-50 text-amber-800 border border-amber-150 p-3.5 rounded-lg text-xs font-medium">
                            ...
                            <span>
                              Absolute Final Deadline for digital files upload:{" "}
                              <strong className="font-mono">
                                {selectedJob.deadline}
                              </strong>
                              . Ensure address proofs are uploaded.
                            </span>
                          </div>
                        ...
                        <div className="space-y-5">
                          ...
                          {applyWizardStep === 1 && (
                            ...
                            >
                              ...
                              <p className="text-slate-450 leading-relaxed font-semibold">
                                ...
                                automatically. You can scan your CV string to
                                autofill communication routes and match skills,
                                or type manually.
                              </p>
                              ...
                              <div className="bg-indigo-50/50 border-2 border-indigo-150 rounded-3xl p-4.5 space-y-3">
                                <div className="flex justify-between items-center">
                                  <div className="flex items-center space-x-2">
                                    ...
                                    <div>
                                      <h5 className="font-extrabold text-indigo-950 text-xs uppercase tracking-wide flex items-center gap-1.5">
                                        ...
                                        <span className="bg-indigo-100 text-indigo-800 text-[9px] px-2 py-0.5 rounded-full font-black">
                                          AI Parse
                                        </span>
                                      </h5>
                                      <p className="text-[10px] text-slate-500 mt-0.5 font-medium">
                                        Scan your CV contents to automatically
                                        map credentials, record phone logs, and
                                  ...
                                  <button
                                    type="button"
                                    onClick={() =>
                                      setShowCvScanner(!showCvScanner)
                                    }
                                  ...
                                  >
                                    ...
                                    <div>
                                      <span className="block text-[10px] font-bold text-indigo-900 mb-1.5">
                                        Draft CV Templates (Click to Auto-load
                                        for testing):
                                      </span>
                                    ...
                                    <div>
                                      <div className="flex justify-between items-center mb-1">
                                        <label
                                          ...
                                          className="block text-[10px] font-extrabold text-indigo-900"
                                        >
                                          Paste CV Contents / Skills Summary
                                          Text:
                                        </label>
                                        ...
                                        {cvScore !== null && (
                                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 mt-1 border-t border-emerald-150">
                                            ...
                                            <div className="bg-white p-2.5 rounded-lg border border-emerald-100">
                                              <p className="text-[9px] text-slate-450 font-bold uppercase tracking-wider mb-1">
                                                Newly Linked Skills
                                              </p>
                                              {extractedSkills.length === 0 ? (
                                                <p className="text-[10px] text-slate-400 font-medium">
                                                  Mapped to registered profile.
                                                </p>
                                              ) : (
                                                <div className="flex flex-wrap gap-1">
                                                  {extractedSkills.map(
                                                    (s, idx) => (
                                                      <span
                                                        key={idx}
                                                        className="bg-indigo-50 text-indigo-800 text-[8px] font-black uppercase px-2 py-0.5 rounded-md border border-indigo-100"
                                                      >
                                                        ✓ {s}
                                                      </span>
                                                    ),
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="md:col-span-2">
                                  ...
                                  <select
                                    ...
                                    onChange={(e) =>
                                      setCitizenEducation(e.target.value)
                                    }
                                    className="w-full p-2.5 border border-slate-250 rounded-lg text-slate-800"
                                  >
                                    <option value="High School Level">
                                      High School Level Credentials
                                    </option>
                                    <option value="Vocational Certificate">
                                      Vocational Training / Trade Certificate
                                    </option>
                                    ...
                                    <option value="Postgraduate Honours">
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    if (!citizenPhone || !citizenEmail) {
                                      alert(
                                        "Phone & Email communication properties are necessary.",
                                      );
                                      return;
                                    }
                          ...
                          {applyWizardStep === 2 && (
                            ...
                            >
                              ...
                              <p className="text-slate-450 leading-relaxed">
                                ...
                                certificates. The hiring agency reviews CVs and
                                address declarations prior to panels scheduling.
                              </p>
                              ...
                              <div className="pt-4 flex justify-between">
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    if (!uploadedId) {
                                      alert(
                                        "National Identity Copy is mandatory.",
                                      );
                                      return;
                                    }
                          ...
                          {applyWizardStep === 3 && (
                            <div
                              className="space-y-4 text-xs animate-fadeIn"
                              id="wizard-step-3"
                            >
                              <h4 className="font-bold text-slate-900 text-sm">
                              ...
                              <p className="text-slate-450 leading-relaxed">
                                ...
                                that files are readable before registering.
                                Double-check deadline bounds.
                              </p>
                              ...
                              <div className="p-3 bg-blue-50 border border-blue-150 rounded-xl text-blue-900 leading-normal font-medium">
                                By pressing "Submit Placement Application" you
                                authorize the National Placement Agency and its
                                departments to verify ID Number clearances with
                                Home Affairs and process document credentials
                                securely.
                              </div>
                          ...
                          {applyWizardStep === 4 && (
                            <div
                              className="space-y-5 text-xs animate-fadeIn text-slate-700"
                              id="wizard-step-4"
                            >
                              <div className="text-center py-4 space-y-2">
                                ...
                                <p className="text-slate-500 max-w-sm mx-auto font-medium text-[11px]">
                                  Your placement application has been securely
                                  registered within the South African national
                                  database system under{" "}
                                  <strong>Rands (R)</strong> compensation
                                  indexes.
                                </p>
                              </div>
                              ...
                              <div className="bg-amber-50/70 border-2 border-amber-200 rounded-2xl p-4.5 space-y-3.5">
                                <div className="flex items-center space-x-2 text-amber-950">
                                  ...
                                  <span className="font-black text-xs uppercase tracking-wide">
                                    Mandatory Return Schedule & Checkpoints
                                  </span>
                                </div>
                                <p className="text-[10px] text-slate-600 leading-normal font-semibold">
                                  South African public placement roles require
                                  strict physical & biometric double-checks. You
                                  must return to this portal on the following
                                  days to verify your position and clear pending
                                  status requirements:
                                </p>
                                    ...
                                    <div className="space-y-0.5">
                                      <div className="flex justify-between items-center">
                                        <span className="font-black text-indigo-950 text-xs">
                                          Checkpoint 1: Home Affairs ID
                                          Cleanliness Inspection
                                        </span>
                                        <span className="bg-indigo-100 text-indigo-900 text-[9px] px-2 py-0.5 rounded-md font-bold font-mono">
                                          Day 5 • 2026-06-22
                                        </span>
                                      </div>
                                      <p className="text-[10px] text-slate-500 leading-normal">
                                        System performs automated DHA validation
                                        checks. Log back to verify your primary
                                        address & citizen ID is marked{" "}
                                    ...
                                    <div className="space-y-0.5">
                                      <div className="flex justify-between items-center">
                                        <span className="font-black text-amber-950 text-xs">
                                          Checkpoint 2: Police Vetting &
                                          Security Clearances
                                        </span>
                                        <span className="bg-amber-100 text-amber-900 text-[9px] px-2 py-0.5 rounded-md font-bold font-mono">
                                          Day 12 • 2026-06-29
                                        </span>
                                      </div>
                                      <p className="text-[10px] text-slate-500 leading-normal">
                                        Check if background reference
                                        certifications have fully synchronized
                                        with placement databases.
                                      </p>
                                    </div>
                                  </div>
                                    ...
                                    <div className="space-y-0.5">
                                      <div className="flex justify-between items-center">
                                        <span className="font-black text-emerald-950 text-xs">
                                          Checkpoint 3: Local Municipal
                                          Interview Scheduling
                                        </span>
                                        <span className="bg-emerald-100 text-emerald-900 text-[9px] px-2 py-0.5 rounded-md font-bold font-mono">
                                          Day 20 • 2026-07-07
                                        </span>
                                      </div>
                                      <p className="text-[10px] text-slate-500 leading-normal">
                                        Final Shortlists announced. Log back to
                                        select your physical face-to-face
                                        evaluation interview timeslot on the
                                        municipal roster.
                                      </p>
                                    </div>
                                  </div>
                              ...
                              <div className="flex bg-indigo-50 border border-indigo-150 rounded-xl p-3 items-center justify-between">
                                <div className="space-y-0.5">
                                  <h5 className="font-bold text-indigo-950 text-[11px]">
                                    Sync Reminders to Phone Calendar?
                                  </h5>
                                  <p className="text-[9px] text-slate-500">
                                    Enable automatic reminder tags for return
                                    checkpoints.
                                  </p>
                                </div>
                                <button
          ...
          <div className="space-y-6" id="citizen-trackers-tab">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                {trackingAppId &&
                myApplications.find((a) => a.id === trackingAppId) ? (
                  (() => {
                    const app = myApplications.find(
                      (a) => a.id === trackingAppId,
                    )!;
                    const job = allJobs.find((j) => j.id === app.jobId);
                    const relatedInterview = myInterviews.find(
                      (i) => i.applicationId === app.id,
                    );

                    return (
                      <div
                        className="bg-white rounded-2xl border border-slate-150 p-6 space-y-6"
                        id="tracker-details-card"
                      >
                        {/* Header block */}
                        ...
                        <div>
                          ...
                          <div className="space-y-6">
                            ...
                            <div className="flex space-x-3.5">
                              ...
                              <div className="text-xs pb-1.5">
                                <span className="font-bold text-slate-900 block text-sm">
                                  Application Clearances Registered
                                </span>
                                <p className="text-slate-450 mt-0.5 leading-relaxed">
                                  Complete profiles submitted successfully on{" "}
                                  {app.appliedDate}. Registered onto clearing
                                  system.
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="flex space-x-3.5">
                              ...
                              <div className="text-xs pb-1.5">
                                <span className="font-bold text-slate-900 block text-sm">
                                  Officer Screening & Backlog Checks
                                </span>
                                <p className="text-slate-450 mt-0.5 leading-relaxed">
                                  {[
                                    "reviewing",
                            ...
                            <div className="flex space-x-3.5">
                              <div className="flex flex-col items-center">
                                <div
                                  className={`h-6 w-6 rounded-full flex items-center justify-center shrink-0 ${
                                    ["interview_scheduled", "placed"].includes(
                                      app.status,
                                    )
                              ...
                              <div className="text-xs pb-1.5 flex-1">
                                ...
                                {relatedInterview ? (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-slate-700">
                                      <div>
                                        ...
                                        <strong className="text-slate-850 font-mono">
                                          {relatedInterview.dateTime.replace(
                                            "T",
                                            " ",
                                          )}
                                        </strong>
                                      ...
                                      <div>
                                        Point of Contact:{" "}
                                        <strong className="text-slate-850">
                                          {relatedInterview.contactPerson}
                                        </strong>
                                      </div>
                                      <div className="md:col-span-2">
                                        Desk/Channel:{" "}
                                  ...
                                  <p className="text-red-700 font-semibold mt-1">
                                    Hiring process terminated by Department
                                    Recruiters.
                                  </p>
                                ) : (
                                  <p className="text-slate-450 mt-0.5 leading-relaxed">
                                    Subject to shortlisting counts. Candidates
                                    will be notified on notification hubs once
                                    scheduled.
                                  </p>
                                )}
                              </div>
                            ...
                            <div className="flex space-x-3.5">
                              ...
                              <div className="text-xs pb-1 flex-1">
                                <span className="font-bold text-slate-900 block text-sm">
                                  Placement Confirmed & Digital letter
                                </span>

                                {app.status === "placed" &&
                                app.placementDetails ? (
                                  <div className="mt-3.5 bg-linear-to-br from-amber-50 to-emerald-50 border border-emerald-150 p-5 rounded-2xl space-y-4">
                                    <div className="flex items-center space-x-1.5 font-black text-emerald-800 uppercase tracking-widest text-[10px]">
                                      ...
                                      />
                                      <span>
                                        Official Civil Placement Clearance
                                      </span>
                                    </div>
                                    ...
                                    <div className="space-y-2 text-slate-700">
                                      ...
                                      <div className="grid grid-cols-2 gap-2 pt-2 text-[11px] border-t border-emerald-100 font-semibold">
                                        <div>
                                          Deployment Date:{" "}
                                          <span className="text-slate-800 block font-bold font-mono">
                                            {app.placementDetails.startDate}
                                          </span>
                                        </div>
                                        <div>
                                          Liaison Officer:{" "}
                                          <span className="text-slate-800 block font-bold">
                                            {app.placementDetails.reportingTo}
                                          </span>
                                        </div>
                                        <div className="col-span-2 mt-1">
                                          Deployment Location:{" "}
                                          <span className="text-slate-805 block font-bold">
                                            {
                                              app.placementDetails
                                                .officeLocation
                                            }
                                  ...
                                  <p className="text-slate-450 mt-0.5 leading-relaxed">
                                    Requires document verification, interviews
                                    completion, and final Bureau sign-off
                                    parameters.
                                  </p>
                                )}
                              </div>
                        ...
                        {app.status === "action_required" && (
                          <div className="p-4 bg-amber-50 border border-amber-150 rounded-xl space-y-3 text-xs">
                            ...
                            <p className="text-slate-650 leading-relaxed">
                              The Department recruitment team has reviews your
                              folder and indicated that certain documents are
                              missing or require update scans. Please check
                              missing markers below to satisfy registration
                              requirements:
                            </p>
                  ...
                  <div className="bg-white rounded-2xl border border-slate-150 p-12 text-center text-slate-400">
                    ...
                    />
                    <p className="font-semibold text-slate-700">
                      Detailed Pipeline Tracker
                    </p>
                    <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                      Select any vacancy query from your submissions list on the
                      left to track real-time evaluations, interviews
                      scheduling, and download official placement credentials.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="flex justify-between items-center bg-white p-5 rounded-2xl border border-slate-150">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  ...
                  required, and employment letters.
                </p>
              </div>
                  ...
                  >
                    <div className="space-y-1">
                      <div className="flex items-center space-x-2 flex-wrap gap-y-1">
                        {!n.isRead && (
                          <span
                            className={`h-2 w-2 rounded-full shrink-0 ${n.type === "alert" ? "bg-rose-600" : "bg-indigo-600"}`}
                          />
                        )}
                        {n.type === "alert" && (
                        ...
                        <h4
                          className={`text-sm font-bold ${n.type === "alert" ? "text-rose-950" : "text-slate-900"}`}
                        >
                          {n.title}
                        </h4>
                      ...
                      <p
                        className={`text-xs pl-4 ${n.type === "alert" ? "text-rose-900 font-medium" : "text-slate-520"}`}
                      >
                        {n.message}
                      </p>
                    </div>
            ...
            >
              ...
              <div className="text-center pb-6 border-b-2 border-slate-800 space-y-1">
                ...
                <p className="text-[10px] text-slate-550 tracking-wider">
                  DEPARTMENT OF PUBLIC SERVICE &amp; ADMINISTRATION &bull; HUMAN
                  RESOURCES PORTAL
                </p>
              ...
              <div className="py-8 space-y-4 text-xs leading-relaxed text-slate-700">
                <p className="font-semibold text-slate-900">
                  To: {printingPlacement.citizenName} (ID:{" "}
                  {printingPlacement.citizenIdNumber})
                </p>
                ...
                <p>
                  We are extremely pleased to deliver this official notification
                  that your clearance profile has been authorized for public
                  placement under current sector parameters. You are assigned to
                  the following position:
                </p>

                <div className="bg-slate-50 p-4 border rounded-lg space-y-1 font-mono text-[11px] text-slate-800 leading-normal">
                  <p>
                    <strong>Opportunity vacancy:</strong>{" "}
                    {
                      allJobs.find((j) => j.id === printingPlacement.jobId)
                        ?.title
                    }
                  </p>
                  <p>
                  ...
                  <p>
                    ...
                    {printingPlacement.placementDetails?.officeLocation}
                  </p>
                  <p>
                    <strong>Effective Deployment Date:</strong>{" "}
                    {printingPlacement.placementDetails?.startDate}
                  </p>
                  <p>
                    <strong>Reporting Liaison Officer:</strong>{" "}
                    {printingPlacement.placementDetails?.reportingTo}
                  </p>
                </div>

                <p className="italic bg-amber-50 p-3 rounded-lg text-amber-900 font-semibold leading-normal">
                  Important Notes:{" "}
                  {printingPlacement.placementDetails?.congratulationsMessage}
                </p>

                <p>
                  Please submit physical ID credentials and original
                  qualification folders upon arrival on{" "}
                  {printingPlacement.placementDetails?.startDate} at{" "}
                  {printingPlacement.placementDetails?.officeLocation} to
                  finalize the standard service payroll registries.
                </p>
              </div>
